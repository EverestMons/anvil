...
The orphan-reconciliation blueprint sizing (2026-06-05) found **2,400 module chunks across only 1,601 unique file_paths** for invoice-pulse — ~799 duplicate module rows. Root cause: the scanner or extractor creates a new module-type `code_chunks` row on each cycle instead of upserting by `(project_id, file_path, chunk_type='module')`. The current orphan-reconciliation plan's prune incidentally cleans duplicates for deleted files (they share the same file_path and get swept by the DELETE), but the root cause persists for surviving files. Not correctness-impacting today — duplicate modules are inert (no findings flow through module chunks directly), and `last_seen_cycle` stamping means only the most-recently-seen row participates in filtered queries. **Fix when picked up:** trace the module-row creation path in `extractor.py` and add an upsert guard (`INSERT OR REPLACE` or existence check) keyed on `(project_id, file_path, chunk_type)`. Then a one-time dedup pass on existing rows, keeping the one with the highest `last_seen_cycle`. **Priority:** Low — cosmetic DB hygiene only.
