...
### 2026-06-03 — Cycle-20 population discontinuity + 5 displaced coupling findings (bundle with sibling scoring bugs)

The phantom fix dropped the scored population from ~5,700+ to 3,688 chunks (cycle 20) — larger than the 5 named phantoms, because the scorer now excludes ALL unstamped orphans (~2,000, mostly from deleted/rebuilt files that were previously scored silently). Two consequences: (1) **5 legitimate cycle-19 coupling findings displaced** from cycle 20 — `execute`, `commit`, `close` (profile_ingestion.py), `_get_db` (action_queue.py), `get_connection` (database.py) — via percentile renormalization on the smaller population. QA accepted these as threshold/ordering shifts but it is *uncharacterized* whether they are genuinely lower-risk now or merely displaced (the same SELF_DECAYED-vs-DISPLACED ambiguity as the volatility-attribution work). (2) **Cross-cycle comparisons now straddle a population break** — cycles ≤19 were percentile-scored against an orphan-polluted population, so cycle-20-vs-19 deltas conflate the fix with real change. **Implication:** `last_seen_cycle` likely *improves* the two sibling scoring bugs (self-resolve, percentile-inversion) by cleaning the population — strong reason to bundle this characterization with that scoring-methodology work. **Fix when picked up:** re-examine the 5 displaced functions against cycle-19 raw scores; quantify the population-base change's effect on percentile stability. **Priority:** Medium — affects trust in cross-cycle deltas.
