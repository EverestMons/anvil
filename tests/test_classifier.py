...
def test_route_handler_by_decorator():
    chunk = {
        "name": "contracts_list",
    ...
    assert classify_chunk(chunk) == "route_handler"


def test_route_handler_by_app_route():
    chunk = {
        "name": "index",
        "file_path": "app.py",
        "content": "@app.route('/')\ndef index():\n    pass",
...
def test_validation_gate_by_name():
    chunk = {
    ...
    assert classify_chunk(chunk) == "validation_gate"


def test_validation_gate_by_higher_number():
    chunk = {
        "name": "gate_10_final",
        "file_path": "engines/validator.py",
...
def test_data_model_by_name():
    ...
    assert classify_chunk(chunk) == "data_model"


def test_data_model_by_migrate_name():
    chunk = {
...
def test_data_model_by_file_path():
    chunk = {
        "name": "some_helper",
...
def test_confidence_engine_by_file_path():
    chunk = {
    ...
    assert classify_chunk(chunk) == "confidence_engine"


def test_content_generator_by_name():
    chunk = {
    ...
    assert classify_chunk(chunk) == "content_generator"


def test_utility_fallback():
    chunk = {
        "name": "_parse_json",
...
def test_fallback_for_unknown_file():
    chunk = {
        "name": "random_helper",
    ...
    assert classify_chunk(chunk) == "utility"


def test_module_returns_none():
    chunk = {
        "name": "app.py",
        "file_path": "app.py",
        "content": "",
    ...
    assert classify_chunk(chunk) is None


def test_test_case_returns_none():
    chunk = {
        "name": "test_something",
...
def test_config_chunk_type():
    ...
    assert classify_chunk(chunk) == "configuration"


def test_ingestion_orchestrator_by_name():
    chunk = {
        "name": "run_ingestion",
    ...
    assert classify_chunk(chunk) == "ingestion_orchestrator"


def test_anomaly_detector_by_name():
    chunk = {
    ...
    assert classify_chunk(chunk) == "anomaly_detector"


def test_pattern_learner_by_name():
    chunk = {
    ...
    assert classify_chunk(chunk) == "pattern_learner"


def test_decorator_priority_over_file_path():
...
def test_name_priority_over_file_path():
...
def test_classify_project(conn):
...
def test_functional_roles_seeded(conn):
    """Verify init_db seeds the 25 functional roles."""
